...
from bf16 import is_bf16, read_tensor
...
# A bfloat16 output uses bf16.bfloat16, the NumPy-only stand-in for ml_dtypes.
case_output_specs = {
    0: [
...
for _cid, _specs in case_output_specs.items():
    for _name, _dtype, _rtol, _atol, _tol in _specs:
        if _dtype is None:
            print(f"ERROR: output '{_name}' of case {_cid} needs a concrete dtype; "
                  "use bf16.bfloat16 for a bfloat16 output.")
            sys.exit(1)

def verify_result(output_path, golden_path, dtype, rtol, atol, tol=0.0):
    output = read_tensor(output_path, dtype)
    golden = read_tensor(golden_path, dtype)
    ...
    if not is_bf16(dtype) and (np.issubdtype(dtype, np.integer) or np.issubdtype(dtype, np.bool_)):
    ...
    # read_tensor already hands bfloat16 back as float32; nothing to widen.
    cmp_output, cmp_golden = output, golden
